...
class Seq2SeqCMTAgent(BaseAgent):
    ...
    def _select_best_action(self, n_candidate_outputs, n_candidate_actions, obs_index, cand_inputs, nav_input, t):
        """
        对 N 个候选动作进行评分并选择最佳动作。
        """
        best_score = -float('inf')
        best_action_index_in_candidates = -1 # N个候选中的索引
        final_action_id = -1 # 最终选出的动作ID (对应环境)

        if not n_candidate_actions: # 如果没有有效的候选动作
             print("Warning: No valid candidate actions generated.")
             # 默认选择停止，或者你可以定义其他回退逻辑
             stop_action_id = cand_inputs['cand_lens'][obs_index] - 1 if not self.args.stop_first else 0
             return 0, stop_action_id # 返回第一个候选的索引和停止动作ID

        for i in range(len(n_candidate_actions)):
            action_id = n_candidate_actions[i]
            action_text = n_candidate_outputs[i] # 假设原始LLM输出文本也需要用于SAS

            # 确保 action_id 是有效的候选动作索引 (非 ignore id)
            # 注意: 这里的 action_id 是 self.prompt_manager.get_output 返回的，可能需要调整以匹配 cand_inputs 的索引
            # 我们需要一个从 get_output 返回的 action_id 到 cand_inputs 中 action 的映射或直接使用 cand_inputs 的索引
            # 假设 get_output 返回的 id 就是 cand_inputs['cand_action'] 中的索引
            # **如果 get_output 返回的不是直接索引，需要在这里转换**
            # 例如，如果 get_output 返回的是环境动作 ID(-1 代表停止)，需要映射回 candidate 列表的索引
            ...
            # 检查索引有效性
            if action_id < 0 or action_id >= len(cand_inputs['cand_action'][obs_index]):
                 print(f"Warning: Skipping invalid action_id {action_id} from LLM output.")
                 continue # 跳过无效的动作 ID

            # --- 计算分数 ---
            sas_score = self._calculate_sas(action_text, nav_input, obs[obs_index], t)
            ...
            if final_score > best_score:
                ...
                final_action_id = action_id # 记录最佳动作的ID
        ...
        print(f"Selected Candidate {best_action_index_in_candidates} with ActionID {final_action_id}, Score: {best_score:.2f}")
        # 返回最佳候选的索引（在N个候选中）和对应的动作ID
        return best_action_index_in_candidates, final_action_id
