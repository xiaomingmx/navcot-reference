...
class Seq2SeqCMTAgent(BaseAgent):
    ...
    def _build_model(self):
        if self.args.llm_predict:
            ...
            # Dynamically import accessory modules
            sys.path.append("/root/autodl-tmp/navcot/NavCoT/LLaMA2-Accessory/accessory")
            from model.meta import MetaModel
            
            # Now, handle the specific loading logic based on provided paths and flags.
            if self.args.use_vllm == True:
                # If using vLLM, the MetaModel's __init__ has already handled initialization.
                # We just confirm it succeeded.
                ...
                self.llm = MetaModel(
                    ...
                    vllm_hf_model_path=self.args.hf_model_path,
                )
                if self.llm.vllm_engine_instance is None:
                    raise RuntimeError("Agent: vLLM was requested, but vLLM engine initialization failed in MetaModel.")
                print("Agent: MetaModel initialized successfully with vLLM engine.")
    ...
    def rollout_llm(self, train_ml=None, reset=True):
        ...
        previous_angle = [{'heading': 0., 'elevation': 0.} for _ in obs]
        ...
        for t in range(self.args.max_action_len):
            ...
            if self.args.ob_type == 'cand':
                print(f"action_timestep: {t} ----------------------\n")

                if self.args.use_vllm:
                    # --- Logic for vLLM Best-of-N ---
                    ...
                    n_candidate_outputs_by_batch, _ = self.llm.generate_with_vllm(
                        nav_input["prompts"],
                        max_gen_len=64,
                    ...
                    a_t = []
                    for i in range(batch_size):
                        # For vLLM, we also use the selection logic now.
                        n_cand_action_txt, n_cand_action_idx = self._extract_action_txt_Idx(
                            n_candidate_outputs=n_candidate_outputs_by_batch[i],
                            bth_idx=i,
                            nav_input=nav_input
                        )
                    ...
                    print(f"a_t (vLLM Best-of-N): {a_t} \n")

                elif hasattr(self.args, 'use_native_best_of_n') and self.args.use_native_best_of_n:
                    # --- Logic for Native (non-vLLM) Best-of-N ---
                    cand_inputs = self._candidate_variable(obs=obs, previous_angle=previous_angle)
                    nav_input = self.prompt_manager.get_prompt(mode='navigation', cand_inputs=cand_inputs, obs=obs, t=t)
                    
                    
                    n_candidate_outputs_by_batch = self.llm.generate_multi_candidate(
                        nav_input["prompts"], images=None, max_gen_len=64,
                        temperature=self.args.best_of_n_temperature,
                        top_p=self.args.top_p if hasattr(self.args, 'top_p') else 0.95,
                        num_candidates=self.n_candidates
                    )

                    print(f"nav_input['prompts'][0]: {nav_input['prompts'][0]} \n")
                    print(f"n_candidate_outputs_by_batch[0]: {n_candidate_outputs_by_batch[0]} \n")

                    a_t = []
                    for i in range(batch_size):
                        n_cand_action_txt, n_cand_action_idx = self._extract_action_txt_Idx(
                            n_candidate_outputs=n_candidate_outputs_by_batch[i], bth_idx=i, nav_input=nav_input
                        )
