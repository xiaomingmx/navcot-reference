...
# Add the parent directory of 'tools' (which is 'accessory') to sys.path
# This allows 'from util.tensor_parallel import ...' to be resolved when
# this script is run directly.
# __file__ is '.../accessory/tools/convert_weights_to_hf.py'
# os.path.dirname(__file__) is '.../accessory/tools'
# os.path.dirname(os.path.dirname(__file__)) is '.../accessory'
_accessory_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _accessory_dir not in sys.path:
    sys.path.insert(0, _accessory_dir)
...
def load_and_merge_tensor_parallel_weights(
    ...
    suffix_to_merge_dim = (
        ...
        ("norm.weight", -1),
        ("rope.freqs", -1),
        # Add entries for biases.
        # Column-parallel layout for q, k, v, and gate proj.
        (".attention.wq.bias", 0),
        (".attention.wk.bias", 0),
        (".attention.wv.bias", 0),
        (".feed_forward.w1.bias", 0),
        (".feed_forward.w3.bias", 0),
        # Row-parallel layout for o_proj and down_proj.
        # These biases are replicated, so they are not merged.
        (".attention.wo.bias", -1),
        (".feed_forward.w2.bias", -1),
    )
    merged_ckpt = {}
    for i, path in enumerate(src_weights_path):
        format, mp_size_val = infer_checkpoint_format_and_mp_size(path)
        
        # Ensure mp_size is an integer for subsequent operations
        mp_size: int
        if isinstance(mp_size_val, int):
            mp_size = mp_size_val
        else:
            try:
                mp_size = int(mp_size_val)
            except ValueError:
                error_message = (
                    f"Error: mp_size '{mp_size_val}' obtained from "
                    f"checkpoint path '{path}' is not a valid integer representation."
                )
                print(error_message)
        ...
        for shard_id in range(mp_size):
            ...
            for key, value in ckpt_shard.items():
                matched_suffix = None
                current_merge_dim = -1 # Default, for non-merged params

                for suffix, merge_dim_val in suffix_to_merge_dim:
                    if key.endswith(suffix):
                        matched_suffix = suffix
                        current_merge_dim = merge_dim_val
                        break
                
                if not matched_suffix:
                    # This parameter is not configured for merging.
                    # This can be due to this parameter not being supported by
                    # HuggingFace's LLaMA implementation, or the merging logic
                ...
                if key not in merged_ckpt:
                    ...
                    if current_merge_dim >= 0: # Use current_merge_dim here
                        merged_size[current_merge_dim] *= mp_size
                    init_dtype = (
                        torch_dtype
                ...
                if key not in sharded_tensor_loaders:
                    sharded_tensor_loaders[key] = ShardedTensorLoader(
                        merged_ckpt[key], mp_size, current_merge_dim, # Use current_merge_dim here
