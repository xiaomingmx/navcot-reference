    ...
    for i, path in enumerate(src_weights_path):
        ...
        for shard_id in range(mp_size):
            ...
            for key, value in ckpt_shard.items():
                ...
                is_known_bias = False
                bias_suffixes = [
                    ".attention.wq.bias", ".attention.wk.bias",
                    ".attention.wv.bias", ".attention.wo.bias",
                ...
                if not matched_suffix:
                    # If no direct match, check if it's a known bias to ignore
                    for bias_suffix in bias_suffixes:
                        if key.endswith(bias_suffix):
                            print(f"Skipping known bias parameter: {key}")
                            is_known_bias = True
                            break
                    if is_known_bias:
                        continue # Skip this parameter

                    # If not a matched suffix and not a known bias, then raise error
