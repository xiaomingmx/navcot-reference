...
def save_checkpoint(output_dir, args, model, optimizer, loss_scaler, dataset_state, epoch=None, iteration=None):
    ...
    with FSDP.state_dict_type(
            ...
            FullStateDictConfig(rank0_only=True, offload_to_cpu=False ),  # 默认offload_to_cpu=True
    ):
        ...
        def _save_model():
            ...
            if getattr(args, "only_save_trainable", False):
                ...
                # --- START LoRA MODIFICATION ---
                # 保存原始的可训练参数，以便处理非LoRA的可训练参数（如bias, norm）
                original_trainable_keys = ['.'.join([_ for _ in key.split('.') if not _.startswith('_')])
                                          for key in model_trainable_params.keys()]
                
                consolidated_model_state_dict = {
                    "model": {key: val.to(save_dtype) for key, val in model.state_dict().items() if key in original_trainable_keys},
                }

                # 专门处理LoRA权重，确保 lora_a 和 lora_b 被单独保存
                # FSDP会给参数名加上前缀，我们需要去掉
                for module_name, module in model.named_modules():
                    # 检查是否是FSDP包装过的LoRA模块
                    if isinstance(module, FSDP) and 'Lora' in module.module.__class__.__name__:
                         # 获取 FSDP unwrap 后的模块
                        unwrapped_module = module.module
                        # 获取 FSDP unwrap 后的模块名
                        clean_module_name = '.'.join([_ for _ in module_name.split('.') if not _.startswith('_')])
                        
                        for sub_name, sub_module in unwrapped_module.named_children():
                            if sub_name in ['lora_a', 'lora_b']:
                                for param_name, param in sub_module.named_parameters():
                                    # 构建完整的键名并保存
                                    full_param_name = f"{clean_module_name}.{sub_name}.{param_name}"
                                    consolidated_model_state_dict["model"][full_param_name] = param.to(save_dtype)
                # --- END LoRA MODIFICATION ---
